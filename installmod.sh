...
install_v2-ui() {
    ...
    wget -N --no-check-certificate -O /usr/local/v2-ui-linux.tar.gz https://github.com/senowahyu62/v2-ui-backup/releases/download/v2-ui-linux.tar.gz/v2-ui-linux.tar.gz
    ...
    echo -e ""
    curl -o /usr/bin/v2-ui -Ls https://raw.githubusercontent.com/dereking/v2-ui-backup/master/v2-ui.sh
    chmod +x /usr/bin/v2-ui
    echo -e "v2-ui 管理脚本使用方法: "
    echo -e "----------------------------------------------"
    echo -e "v2-ui              -Menampilkan menu manajemen"
    ...
    echo -e "v2-ui enable       -Atur v2-ui untuk memulai secara otomatis setelah booting"
    echo -e "v2-ui disable      -Batalkan boot v2-ui dari awal"
...
source ~/.bashrc
echo -e "Tool Install cert"
echo -e "Masukkan Nama Domain"
read -e domain
echo -e "Masukkan Nama Untuk Cert"
read -e nama
cd /root/.acme.sh
./acme.sh --register-account -m senowahyu62@gmail.com
./acme.sh --issue --standalone -d $domain
./acme.sh --installcert -d $domain --fullchainpath /etc/v2ray/$nama.crt --keypath /etc/v2ray/$nama.key
